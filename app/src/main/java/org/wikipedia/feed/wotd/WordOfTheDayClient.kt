    ...
    override fun request(context: Context, wiki: WikiSite, age: Int, cb: FeedClient.Callback) {
        ...
        clientJob = coroutineScope.launch(
            ...
            val deferredCards = WikipediaApp.instance.languageState.appLanguageCodes
                .filter { !FeedContentType.WORD_OF_THE_DAY.langCodesDisabled.contains(it) }
                .map { langCode ->
                    async {
                        val wikiSite = WikiSite.forLanguageCode(langCode)
                        try {
                            WordOfTheDayParser.fetch(wikiSite, age)?.let { WordOfTheDayCard(it, age, wikiSite) }
                        } catch (e: Exception) {
                            L.v(e)
                            null
                        }
